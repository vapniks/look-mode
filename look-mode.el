...
;; Created: 2009-01-08
;; Version: 2.0
;; Last-Updated: 2015-11-08 23:04:25
;;           By: Joe Bloggs
;; URL: https://github.com/vapniks/look-mode
...
;;
;; The file list is generated by expanding a wilcard expression such as
;; /home/me/Pictures/*.jpg
;; One moves through the file list using the keybindings:
...
;;; Usage:
;;
;; M-x look-at-files RET
;; then enter a wildcard expression matching the files you want
;; to look at.
;; 
...
;; To exclude files from the list see `look-skip-file-list'
;; and `look-skip-directory-list'. 
;; For some types of files such as images and documents, you may
;; wish to perform some operations after they are loaded, such as
;; resizing or jumping to a particular page. This can be achieved
;; by setting `look-default-file-settings' to specify default actions
;; for different file types. If the current file is a document or
;; image then when you switch to the next file in the list any changes
;; that were made (such as resizing or changing page) will be remembered
;; and applied next time the file is visited. The information that is
;; saved, and the actions taken when the file is revisited can be set
;; by customizing `look-file-settings-templates'. You can also specify
;; actions for other file types. By default altered image files are
;; cached so that the altered image can be quickly recreated when revisited.
;; If you are low on disk space you can reduce the size of the cache
;; directory with the `look-cache-directory-size' option, or turn off
;; caching altogether with the `look-cache-images' option.
;; 
;; The file list can be sorted and filtered in various ways with the `look-sort-files' 
;; and `look-filter-files' commands (bound to "C-c ^" and "C-c /" respectively).
;; You can add more sorting and filtering functions by customizing `look-sort-predicates'
;; and `look-filter-functions'.
;;
;; For other commands and options, see the list below:
;;;;
...
;;    Re-initializes look-mode's variables. Bound to 
;;  `look-at-files'
...
;;; Installation:
;;
;; Put look-mode.el in a directory in your load-path, e.g. ~/.emacs.d/
...
;; (require 'look-mode)
;;
;; Note: eimp gets loaded if images are identified
...
;;    default = 100000000
;;  `look-default-file-settings'
;;    Alist of default values for `look-file-settings'.
;;  `look-sort-predicates'
...
;;      M-x customize-group RET look-mode RET
;;

;;; Change log:
;;	
;; 2015/11/08: lots of improvements - multiple buffers, cacheing, etc.
;;
...
;;; Requirements:
(require 'ido-choose-function)		;available here: https://github.com/vapniks/ido-choose-function
(require 'cl-lib)			;comes with emacs
...
Throw an error if it's not."
  (unless look-mode (error "Current buffer is not a `look-mode' buffer")))
...
;; (magit-push)
;; (yaoddmuse-post "EmacsWiki" "look-mode.el" (buffer-name) (buffer-string) "update")
